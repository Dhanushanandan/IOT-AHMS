...
void resetForNextReading() {
    ...
    // Draw the filled heart shape below the message
    drawFilledHeart(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 10, 20); // Display a filled heart shape with size 20
    display.fillCircle(52, 35, 10, SSD1306_WHITE); // Circle moved more left and smaller
    display.fillCircle(77, 34, 10, SSD1306_WHITE); // Circle moved more left and smaller
    display.fillCircle(60, 35, 10, SSD1306_WHITE); // Circle moved more left and smaller
...
// Function to draw a filled heart shape
void drawFilledHeart(int x, int y, int size) {
    // Top part of the heart (two filled circles)
    display.fillCircle(x - size / 2, y - size / 2, size / 2, SSD1306_WHITE);  // Left circle
    display.fillCircle(x + size / 2, y - size / 2, size / 2, SSD1306_WHITE);  // Right circle

    // Bottom part of the heart (triangle)
    display.fillTriangle(x - size, y, x, y + size, x + size, y, SSD1306_WHITE); // Bottom triangle
}
...
void loop() {
    ...
    if (fingerDetected && !waitingForReadings && !displayingResult) {
        ...
        drawFilledHeart(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 10, 20); // Display a filled heart shape with size 20
        display.fillCircle(52, 35, 10, SSD1306_WHITE); // Circle moved more left and smaller
        display.fillCircle(77, 34, 10, SSD1306_WHITE); // Circle moved more right and smaller
        display.fillCircle(60, 35, 10, SSD1306_WHITE); // Circle moved more right and smaller
        display.display();
    }
