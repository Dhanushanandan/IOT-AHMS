...
#define ABNORMAL_ECG_THRESHOLD 600  // Threshold for abnormal ECG reading (adjust as needed)
...
void printTemperature(int averageHeartRate, float averageSpO2, float averageSystolicBP, float averageDiastolicBP) {
  ...
  // Send more sensor data (e.g., averageHeartRate, averageSpO2, averageSystolicBP, averageDiastolicBP)
  ...
  storeSensorData(averageHeartRate, averageSpO2, tempC, averageSystolicBP, averageDiastolicBP, temp, hum);
  delay(5000);
}
...
void storeSensorData(int HeartRate, float SpO2, float tempC, float averageSystolicBP, float averageDiastolicBP, float temp, float hum) {
  // Check if file exists, create if not
  if (!SD.exists("Data.txt")) {
    Serial.println("Data.txt not found, creating...");
    File createFile = SD.open("Data.txt", FILE_WRITE);
    if (createFile) {
      Serial.println("Data.txt created successfully.");
      createFile.close();
    } else {
  ...
  if (dataFile) {
    ...
    dataFile.print("Body Temperature: ");
    ...
    dataFile.print("Room Temp: ");
    dataFile.print(temp);
    dataFile.print(" °C, ");

    dataFile.print("Systolic BP: ");
    dataFile.print(averageSystolicBP);
    dataFile.print(" mmHg, ");

    dataFile.print("Diastolic BP: ");
    dataFile.print(averageDiastolicBP);
    dataFile.println(" mmHg");

    dataFile.close();
    Serial.println("Sensor data successfully stored in SD card.");
  } else {
    Serial.println("Error opening SensorData.txt! Check SD card.");
  }
}
